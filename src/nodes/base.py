...
class Node:
    ...
    def parse_data(self, data: list[str]) -> Attributes:
        positionals = reversed(param for param in self.params if not param.endswith('?') and not param.endswith('='))
        boolean = {param.removesuffix('?') for param in self.params if param.endswith('?')}
        named = {param.removesuffix('=') for param in self.params if param.endswith('=')}
        data_dict = {}
        for arg in data:
