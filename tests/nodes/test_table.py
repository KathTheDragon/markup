...
class Test_table_node:
    ...
    def test_only_accepts_headers_data_arg():
        assert table.table_node(data=['headers=rows']) == '<table></table>'
        with raises(table.InvalidData):
            table.table_node(data=['foo'])

    def test_headers_data_arg_can_only_have_rows_and_cols():
        with raises(table.InvalidData):
            table.table_node(data=['headers=foo'])

    def test_caption_can_be_added_before_table_body_with_double_slash():
        assert table.table_node(text=['foo', '//', 'bar']) == '<table><caption>foo</caption><tr><td>bar</td></tr></table>'

    def test_leading_and_trailing_whitespace_around_caption_placed_outside_tag():
        assert table.table_node(text=['  \n  ', 'foo', '\t\r\t', '//', 'bar']) == '<table>\n  <caption>foo</caption>\r\t<tr><td>bar</td></tr></table>'

    def test_table_rows_separated_by_single_slash():
        assert table.table_node(text=['foo', '/', 'bar', '/', 'baz']) == '<table><tr><td>foo</td></tr><tr><td>bar</td></tr><tr><td>baz</td></tr></table>'

    def test_table_cells_separated_by_pipe():
        assert table.table_node(text=['foo', '|', 'bar', '|', 'baz']) == '<table><tr><td>foo</td><td>bar</td><td>baz</td></tr></table>'

    def test_rows_must_all_contain_the_same_number_of_cells():
        assert table.table_node(text=['foo', '|', 'bar', '/', 'baz', '|', 'bif']) == '<table><tr><td>foo</td><td>bar</td></tr><tr><td>baz</td><td>bif</td></tr></table>'
        with raises(table.MarkupError):
            table.table_node(text=['foo', '|', 'bar', '/', 'baz'])

    def test_leading_and_trailing_whitespace_trimmed():
        assert table.table_node(text=['  ', 'foo', '   ']) == '<table><tr><td>foo</td></tr></table>'
    ...
    def test_first_row_is_heading_cells_if_cols_in_header_data_arg():
        assert table.table_node(data=['headers=cols'], text=['foo', '|', 'bar', '/', 'baz', '|', 'bif']) == '<table><tr><th>foo</th><th>bar</th></tr><tr><td>baz</td><td>bif</td></tr></table>'

    def test_first_column_is_heading_cells_if_rows_in_header_data_arg():
        assert table.table_node(data=['headers=rows'], text=['foo', '|', 'bar', '/', 'baz', '|', 'bif']) == '<table><tr><th>foo</th><td>bar</td></tr><tr><th>baz</th><td>bif</td></tr></table>'

    def test_first_row_and_column_are_heading_cells_if_rows_and_cols_in_header_data_arg():
        assert table.table_node(data=['headers=rows,cols'], text=['foo', '|', 'bar', '/', 'baz', '|', 'bif']) == '<table><tr><th>foo</th><th>bar</th></tr><tr><th>baz</th><td>bif</td></tr></table>'
